  ...
  async initialize(): Promise<void> {
    try {
      ...
      try {
        ...
        console.log('Provider connected to Hardhat network:', network.name, 'chainId:', network.chainId.toString());
        
        // Check if we're connected to the expected local network
        const expectedChainId = process.env.EXPO_PUBLIC_CHAIN_ID || '1337';
        if (network.chainId.toString() !== expectedChainId) {
          console.warn(`Warning: Connected to chain ${network.chainId} but expected ${expectedChainId}`);
        }
      } catch (providerError) {
        console.error('Provider connection failed:', providerError);
        throw new Error(`Failed to connect to Hardhat network. Make sure Hardhat node is running with: npx hardhat node`);
      }
