...
const ProfileScreen = () => {
  const { address, resetWallet, connected, smartAccountAddress } = useAddress();
  ...
  const updateUserDetails = async () => {
    try {
      ...
      const data = contractInterface.encodeFunctionData('updateUser', [editedName, editedEmail]);
  ...
  const disconnectWallet = async () => {
    try {
      await resetWallet();
      Alert.alert('Success', 'Wallet reset successfully');
    } catch (error) {
  ...
  const renderUserInfo = () => (
    <View style={styles.infoContainer}>
      ...
      <View style={styles.infoRow}>
        ...
        <Text style={styles.infoValue}>{address ? `${address.slice(0, 6)}...${address.slice(-4)}` : 'Not connected'}</Text>
      </View>
    </View>
  );
