...
const HomeScreen: React.FC = () => {
  ...
  const connectWallet = async () => {
    try {
      if (address) {
        ...
        // Initialize Account Abstraction if available
        if (!isAAInitialized && walletProvider) {
          try {
            const provider = new BrowserProvider(walletProvider);
            const signer = await provider.getSigner();
            await initializeAA(signer);
            setUseGaslessTransaction(true);
            addToActivityLog('Account Abstraction initialized');
          } catch (aaError) {
            console.log('AA initialization failed:', aaError);
            addToActivityLog('Account Abstraction not available');
          }
        }
      }
    } catch (error) {
  ...
  const registerUser = async () => {
    ...
    try {
      ...
      if (useGaslessTransaction && isAAInitialized && gaslessEnabled) {
        try {
          const { accountAbstractionService } = await import('../services/AccountAbstractionService');
          ...
          console.log('Gasless transaction failed, falling back to regular transaction:', gaslessError);
          addToActivityLog('Gasless transaction failed, using regular transaction');
        }
      }
      ...
      if (event) {
        ...
        const [userAddress, newSimNumber] = parsed?.args || [];
